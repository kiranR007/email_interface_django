...
def upload_the_file(file,campaign_id):
    ...
    path = "/var/www/demosendy/uploads/attachments/"
    ...
    #     print("sftp connected")
        ...
        if not os.path.exists(sftp_target_path):
            os.makedir(sftp_target_path)
    
        subprocess.run(["cp", file, sftp_target_path], check=True)

        print("file uploaded")
    except subprocess.CalledProcessError as e:
        print(f"Failed to copy the file. Error: {str(e)}")
